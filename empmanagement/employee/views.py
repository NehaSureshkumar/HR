...
def leave_request_list(request):
    try:
        ...
        if request.user.is_staff:
            ...
            # For employees: show only their leave requests
            requests = LeaveRequest.objects.filter(eId=request.user.username).order_by('-created_at')
        
        # Attach employee name for template
        for req in requests:
            try:
                employee = Employee.objects.get(eID=req.eId)
                req.employee_name = f"{employee.firstName} {employee.lastName}"
            except Employee.DoesNotExist:
                req.employee_name = req.eId
